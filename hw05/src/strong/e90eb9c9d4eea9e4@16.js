...
function _simple1(d3,output,drag,url_list,invalidation)
{
  // 指定圖表的尺寸。
  const width = 1500
    ;
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["output@1.json", {url: new URL("../outputSimple.json", import.meta.url), mimeType: "application/json", toString}]
  ]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer()).define(["md"], _1);
  main.variable(observer("simple1")).define("simple1", ["d3","output","drag","url_list","invalidation"], _simple1);
